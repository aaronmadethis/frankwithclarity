...
const bootstrappath = 'node_modules/bootstrap/scss/'; // path to find bootstrap scss
...
// Combine require into one file served up in browser
...
gulp.task( 'styles', () => {
	return gulp
		.src([ config.styleSRC, config.styleLoginSRC ], { allowEmpty: true })
 ...
 *     2. Looks for moderizr tests
 *     3. creates a modernizr file in destination folder
 */
gulp.task('modernizr', function() {
	return gulp
		.src( config.jsCustomSRC, { since: gulp.lastRun( 'modernizr' ) }) // Only run on changed files.
		.pipe(modernizr())
		.pipe( gulp.dest( config.ModernizrDestination ) )
		.pipe( notify({ message: '\n\n✅  ===> MODERNIZR — completed!\n', onLast: true }) );
  });
 ...
 *     4. Uglifes/Minifies the JS file and generates custom.min.js
 */
gulp.task( 'frankCustomJS', () => {
	return browserify({
		entries: './assets/js/custom/app.js',
		debug: true,
...
gulp.task(
	...
	gulp.parallel( 'styles', 'vendorsJS', 'frankCustomJS', 'images', browsersync, () => {
		gulp.watch( './*.html', reload ); // Reload on template PHP files
		gulp.watch( config.watchPhp, reload ); // Reload on PHP file changes.
		gulp.watch( config.watchStyles, gulp.parallel( 'styles' ) ); // Reload on SCSS file changes.
		gulp.watch( config.watchJsVendor, gulp.series( 'vendorsJS', reload ) ); // Reload on vendorsJS file changes.
		gulp.watch( config.watchJsCustom, gulp.series( 'frankCustomJS', reload ) ); // Reload on customJS file changes.
